...
export default async function Footer() {
  ...
  try {
    menuItems = await getMenuItems('secondary-navigation');
  } catch (error) {
    console.error("Failed to fetch secondary navigation menu:", error);
  }
      ...
      <div className="max-w-6xl mx-auto px-4 py-12" suppressHydrationWarning>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8" suppressHydrationWarning>
          ...
          <div suppressHydrationWarning>
            ...
            <p className="text-zinc-600 dark:text-zinc-400 text-sm">
              A modern blog powered by WordPress and Next.js, delivering fast and
              seamless reading experience.
            </p>
          </div>

          {/* Quick Links - from WordPress menu or fallback */}
          <div suppressHydrationWarning>
            <h3 className="text-lg font-bold text-black dark:text-white mb-4">
              Quick Links
            </h3>
            <ul className="space-y-2">
              {menuItems && Array.isArray(menuItems) && menuItems.length > 0 ? (
                ...
                /* Fallback links if menu not available */
                <>
                  <li>
                    <Link
                      href="/"
        ...
        <div className="mt-8 pt-8 border-t border-zinc-200 dark:border-zinc-800 text-center" suppressHydrationWarning>
          <p className="text-sm text-zinc-600 dark:text-zinc-400">
            © {currentYear} Felix Seeger. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
