...
export default function AuthorCard({
  ...
  const link = authorLink || `/author/${author.slug}`;
  ...
  if (variant === 'horizontal') {
    return (
      <div className={`flex items-start gap-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg ${className}`}>
        {avatarUrl && (
  ...
  return (
    <div className={`p-6 bg-gray-50 dark:bg-gray-800/50 rounded-xl text-center ${className}`}>
      {avatarUrl && (
        <Link href={link} className="inline-block mb-4">
          <Image
            src={avatarUrl}
            alt={author.name}
            width={80}
      ...
      <Link
        href={link}
        className="block text-xl font-semibold text-gray-900 dark:text-gray-100 hover:text-blue-600 dark:hover:text-blue-400 mb-2"
      >
        {author.name}
      </Link>
